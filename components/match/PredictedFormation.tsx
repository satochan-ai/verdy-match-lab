  ...
  return (
    <section aria-labelledby={`predicted-formation-${team.id}`}>
      <div className="flex items-baseline justify-between gap-3 border-b border-border pb-2">
        <h3 id={`predicted-formation-${team.id}`} className="min-w-0 text-[13px] font-bold text-text-primary">
          {team.name}
        </h3>
      ...
      <div
        aria-hidden="true"
        className="relative mt-2 overflow-hidden border border-border bg-surface px-2 py-3"
      >
        <div className="pointer-events-none absolute inset-x-2 top-1/2 border-t border-border" />
        <div className="pointer-events-none absolute left-1/2 top-1/2 size-11 -translate-x-1/2 -translate-y-1/2 rounded-full border border-border" />
        <div className="pointer-events-none absolute inset-x-[18%] top-0 h-6 border-x border-b border-border" />
...
function PlayerMarker({
  ...
  return (
    <div className="min-w-0 text-center">
      <span
        className={`mx-auto flex size-5 items-center justify-center rounded-full text-[10px] font-bold tabular-nums ${
          team.isVerdy ? "bg-primary-green text-white" : "border border-text-secondary bg-surface text-text-primary"
        }`}
      >
        {player.number ?? "—"}
      </span>
      <p className="mx-auto mt-0.5 max-w-[5.5rem] truncate text-[10px] font-bold leading-tight text-text-primary">
        {player.name}
      </p>
    </div>
  );
