...
/**
 * BELEZA第2節の実際の並び（Phase 6-B.2 / 左右反転修正 6-B.2b）。ユーザー確認済みの配置に基づき
 * 4-2-3-1 としてフォーメーション図表示に用いる。belezaActualLineup（GK/DF/MF/FWの区分のみ・
 * 公式画像確認）とは別に、画面上の5ライン（GK / DF4 / ボランチ2 / 2列目3 / FW1）で見せるための
 * 順序付き配列。
 *
 * startersはFormationPitchが要求する順序（GK → DF → ボランチ → 2列目 → FW）で、
 * 各行内の配列順＝画面左→右。ユーザー指定の「右から」の順を画面左→右へそのまま置くと
 * 左右が反転するため、6-B.2bで各行を反転済み：
 *   DF   ＝左から 32 松岡→3 村松→4 土光→22 井手
 *   ボランチ＝左から 35 須長→6 隅田
 *   2列目 ＝左から 7 北村→19 塩越→13 氏原
 * position名はGK/DF/MF/FWのまま（ボランチ/2列目の区別は行分割で表現する）。
 * 選手11名・背番号・氏名・formationはbelezaActualLineup / 6-B.2と一致させ、変更しない。
 */
