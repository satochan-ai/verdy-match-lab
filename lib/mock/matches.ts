...
export const matches: Match[] = [
  ...
  },
  {
    ...
    // 千葉戦（match-13）が終了したため、東京V側の予想スタメンも今回から登録する
    // （千葉戦actual lineupをベースに、starting XI・出場時間等を踏まえた予想）。
    id: "match-14",
    ...
    alternativeFormations: [
      ...
      },
      {
        ...
        formation: "4-1-2-3",
        starters: [
          { role: "GK", name: "西川 周作", alternative: "福井 光輝" },
          { role: "DF", name: "長沼 洋一" },
          { role: "DF", name: "根本 健太" },
          { role: "DF", name: "宮本 優太" },
          { role: "DF", name: "山根 視来", alternative: "林 幸多郎" },
          { role: "アンカー", name: "瀬古 樹" },
          { role: "インサイド", name: "渡邊 凌磨", alternative: "南野 遥海" },
          { role: "インサイド", name: "安居 海渡", alternative: "植木 颯" },
          { role: "FW", name: "マテウス サヴィオ", alternative: "南野 遥海" },
          { role: "FW", name: "小森 飛絢", alternative: "オナイウ" },
          { role: "FW", name: "金子 拓郎", alternative: "水沼 宏太" },
        ],
        note: "ボザ欠場予定によるCB不足を踏まえた対抗案。山根がSBに入り、瀬古がアンカーとして中盤の基準点になる。",
      },
      {
        title: "対抗案C：4-2-3-1",
        formation: "4-2-3-1",
        starters: [
          ...
          { role: "DF", name: "山根 視来", alternative: "林 幸多郎" },
          { role: "ボランチ", name: "瀬古 樹" },
          { role: "ボランチ", name: "植木 颯", alternative: "安居 海渡" },
          { role: "右", name: "金子 拓郎", alternative: "水沼 宏太" },
          { role: "トップ下", name: "渡邊 凌磨", alternative: "南野 遥海" },
          { role: "左", name: "マテウス サヴィオ", alternative: "南野 遥海" },
          { role: "CF", name: "小森 飛絢", alternative: "オナイウ" },
        ],
        note: "瀬古の相方を植木・安居のどちらにするかで中盤の運び方が変わる。CB事情を踏まえると、今回はかなり有力な対抗案として見ている。",
      },
    ],
    availability: {
      // ダニーロ ボザ：岡山戦で前半15分に負傷交代しており、今回は欠場予定として扱う。
    ...
    matchNotes: [
      // 東京V前節（千葉戦）レビュー。ユーザー確認済みの試合内容をベースに、
      // 「東京Vがどんな状態で浦和戦へ入るのか」を先に置く構成（既存の浦和側短評は
      // 削除せず、この5段落の後にそのまま続ける）。
      "前節の千葉戦は、東京Vが主導権を握る時間が長く、相手陣内まで何度も押し込んだ。ただ、相手GKのビッグセーブに阻まれ、シュートも最後の精度を欠いて得点にはつながらない。流れをつかんでいた中、千葉に一瞬の隙を突かれ、矢村のシュートをマテウスが防いだものの、こぼれ球がエリソンの前へ。押し込まれて先制を許し、ゲームプランが難しくなった。",
      "後半は新加入の一美を投入し、染野以外にも前線の起点を作った。終盤には鈴木に代えて松橋を入れ、4-4-2へ変更。右から松橋、林、井上、溝口の4バック、熊取谷、食野、平川、新井の中盤、染野と一美の2トップでさらに攻勢を強めた。",
      "アディショナルタイム6分、CKから溝口のクロスを林が頭で押し込み、ようやく同点。その後も勝ち越しを狙ったが、1-1で終了した。",
      "残留争いの直接対決は痛み分け。両チームにとって勝点1では物足りない結果となった。東京Vはこれで開幕から7戦未勝利となり、20クラブで唯一まだ勝利がない。ただ、千葉戦は内容そのものが悪かったわけではなく、むしろ流れをつかめていた試合だった。だからこそ、先制点を許したことと、決めるべき場面で決め切れなかったことが重く残る。",
      "浦和戦でもまず欲しいのは先制点。そして何より、そろそろ“初日”――今季初勝利が欲しい。",
      "浦和は岡山戦で鹿島戦から先発を6人変更した。西川、金子、林は体調面で不透明な状況があり、渡邊もコンディションが万全とは言い切れない。一方で、ダニーロ・ボザは岡山戦で負傷交代しており、現時点では欠場を想定している。",
      "メンバーが大きく変わっても、岡山戦では3-4-2-1を継続した。瀬古を中央に置く形も変わっておらず、浦和が上向いてきた中で瀬古が基準になっているという見方は変わらない。",
      "ただし、センターバックの人数は多くない。ボザが欠場する場合、3枚を使う3バックを続けるのか、4バックへ戻してCBを2枚にするのかは大きなポイントになる。",
      "4バックなら4-1-2-3だけでなく、瀬古と植木または安居を並べる4-2-3-1も考えられる。岡山戦で先発した南野は複数ポジションで使えるため、今回の予想では控え以上の存在として見ている。",
      "東京Vとしては、浦和のフォーメーションそのものだけでなく、瀬古がどこに立つか、そして瀬古が下がった後に浦和の中盤がどう変わるかまで見ておきたい。",
    ],
    focusPoints: [
      "3バック継続か、4バックへ戻すか：ボザの欠場予定とCB層を考えると、岡山戦まで続けた3バックを継続するか、4バックへ戻すかは今回の大きな注目点。",
      "瀬古がいる時間と、いなくなった後：浦和が上向いてきた中で瀬古の存在は大きい。フル出場できる状態かは不透明で、途中交代した後に中盤がどう変わるかまで見たい。",
      "南野をどこで使うか：岡山戦で先発した南野は、インサイド、トップ下、サイドと複数ポジションで使える。先発継続か、途中投入かも含めて今回のキーマン候補。",
    ],
    strategies: [
      {
        orderNo: 1,
